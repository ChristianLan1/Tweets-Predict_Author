...
classifier = OneVsRestClassifier(LinearSVC(dual = False))
...
print("start fitting liner svc:"+ datetime.datetime.now().strftime(ISOTIMEFORMAT))
...
df.index = df.index + 1
df.index.name = 'Id'
df.to_csv('combined TF-IDFwith logistic.csv')
